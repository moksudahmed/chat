...
class Helper{
	...
	async registerUser(params){
		try {
			const userexist = await this.db.query(`SELECT id,username FROM user WHERE LOWER(username) = ?`, [params.username]);
			if(userexist.length) return 'User exists';
	...
	async loginUser(params){
		try {
			return await this.db.query(`SELECT id,username FROM user WHERE LOWER(username) = ? AND password = ?`, [params.username,params.password]);
		} catch (error) {
			return null;
		}
	}
